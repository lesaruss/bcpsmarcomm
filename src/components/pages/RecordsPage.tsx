...
// Records is the standalone home for personal/confidential documents -
// performance appraisals, evaluations, time-off requests, contracts - split
// out of the old shared Documents page (2026-08-19, per Sean/V: "Records
// should also be by themselves on the left and house more personal
// documents like time off requests, evaluations, etc." and to remove the
// confusion of it living as a tab buried inside Documents). Reuses the same
// /api/bcps/documents endpoint and acl_objects-backed access model as
// Documents and Meeting Notes - just permanently filtered to section='records'.
export default function RecordsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setDocs((j.documents ?? []).filter((d: Doc) => (d.section || 'documents') === 'records'))
    setRole(j.role)
    ...
    <div style={{ padding: '0' }}>
      ...
      <div className="docs-section">
        <div className="docs-header">
          <h1>Records</h1>
          <p>Performance appraisals, evaluations, and confidential employment records.
            {isAdmin && ' As an admin, each record also shows who has access and an Edit button to manage access or content directly here.'}
          </p>
        </div>
              ...
              return (
                <div key={doc.id} className="doc-card">
                    ...
                    <div className="doc-badge-row">
                      ...
                      {isAdmin && (
                        <button
                          ...
                          title={doc.featured ? 'Remove from featured' : 'Feature this record'}
                        >
                          {doc.featured ? '★' : '☆'}
                        </button>
                      )}
                  ...
                  <div className="doc-meta">
                    <span className="doc-type-record">{doc.type}</span>
                    <span>{doc.date}</span>
                  </div>
                  ...
                  {openPanel === 'content' && isAdmin && (
                    <div className="doc-panel">
                      ...
                      {doc.is_dynamic ? (
                        ...
                        <div style={{ fontSize: 12, color: '#9ca3af' }}>This record isn&apos;t database-backed yet and can&apos;t be edited here.</div>
                      )}
                    </div>
                  )}
                    ...
                    <div className="doc-panel">
                      ...
                      <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 8 }}>
                        Describe what you&apos;d like changed, in plain language. {doc.can_edit && !doc.sensitive
                          ...
                          : doc.sensitive
                            ? "This is a sensitive record, so it always goes to the owner for approval first, then into the agent queue."
                            : "This goes to the record's owner for approval, then into the agent queue."}
                      </div>
                      <textarea
                        value={requestDraft[doc.slug] ?? ''}
                        onChange={e => setRequestDraft(prev => ({ ...prev, [doc.slug]: e.target.value }))}
                        placeholder="e.g. Update my evaluation date."
