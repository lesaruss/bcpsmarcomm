...
// components/pages/SchoolsAdaPage.tsx
//
// Tracks each school's ADA score across their WHOLE site, not just one
// page. Modeled on DepartmentsPage's score-ring/health-status pattern.
// "Scan Full Site" discovers a school's pages via sitemap.xml (capped,
// see lib/sitemap-crawl.ts), then scans each one through the existing
// proven /api/bcps/ada-scan pipeline (axe-core + WAVE + Lighthouse, no LLM
...
//
// Per Sean 2026-08-28: on-demand button per school for v1, not a scheduled
// sweep (WAVE API credits are metered - a scheduled full-district crawl
// needs a credit budget check first).
...
export default function SchoolsAdaPage() {
  ...
  const scanFullSite = async (school: School) => {
    if (!school.site_url) { setError(`${school.name} has no website URL on file. Add one on the School ADA Accounts page first.`); return }
    setError('')
    const t = await token()
  ...
  return (
    <div>
      <div style={C.card}>
        <div style={C.sublabel}>Schools ADA</div>
        <p style={{ fontSize: 13, color: '#4b5563', margin: 0 }}>
          Full-site accessibility score per school, not just one page. &ldquo;Scan Full Site&rdquo; discovers a
          school&apos;s pages from its sitemap.xml (capped for now) and runs the same proven axe-core + WAVE +
          Lighthouse pass on each one, then averages them into one score below.
        </p>
      ...
      {loading ? (
        ...
        <div style={C.card}>
          <div style={{ fontSize: 13, color: '#9ca3af' }}>
            No schools onboarded yet. Add schools on the School ADA Accounts page first, this page tracks
            scores for schools that already exist there.
          </div>
        </div>
      ) : (
