    ...
    <div>
      <div style={C.card}>
        ...
        <p style={{ fontSize: 13, color: '#4b5563', margin: '0 0 14px' }}>
          ...
          browardschools.ai) and run a real accessibility check, powered by Google&apos;s Lighthouse engine.
          Same engine Chrome DevTools uses - no guessing, no placeholder data.
        </p>
        ...
        <div style={C.card}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 10, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.06em' }}>ADA Score</div>
              </div>
              {(() => {
          ...
          {result.ada_violations.length === 0 ? (
            <div style={{ fontSize: 13, color: '#059669', fontWeight: 600 }}>
              No failing accessibility checks found by Lighthouse on this page. 🎉
            </div>
          ) : (
            <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {result.ada_violations.map(v => (
                  <div key={v.id} style={{ borderLeft: `3px solid ${SEVERITY_COLORS[v.severity]}`, paddingLeft: 12, paddingTop: 2, paddingBottom: 2 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: '#111827' }}>{v.title}</div>
                    <div style={{ fontSize: 12, color: '#6b7280', marginTop: 2 }}>{v.description.replace(/\[.*?\]\((.*?)\)/g, '')}</div>
      ...
      <div style={C.card}>
        <div style={C.sublabel}>Recent scans (this district)</div>
        {historyLoading ? (
